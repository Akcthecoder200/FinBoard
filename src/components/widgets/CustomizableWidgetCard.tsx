          ...
          <div className="grid grid-cols-2 gap-4 pt-3 border-t border-border">
            {data.volume && (
              <div>
                ...
                <div className="text-sm font-medium">{(data.volume / 1e6).toFixed(1)}M</div>
              </div>
            )}
            {data.marketCap && (
              <div>
                <div className="text-xs text-muted-foreground">Market Cap</div>
                <div className="text-sm font-medium">${(data.marketCap / 1e9).toFixed(1)}B</div>
              </div>
            )}
          </div>
        )}
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {Object.entries(data.indices).slice(0, 3).map(([key, indexData]) => {
  ...
  const renderWidgetContent = () => {
    ...
    switch (widget.type) {
      ...
      case 'table':
        return (
          <TableWidget
            symbols={widget.config?.symbols as string[] || ['AAPL', 'GOOGL', 'MSFT']}
