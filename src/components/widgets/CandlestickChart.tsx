...
const CandlestickBar: React.FC<{
  ...
  // Scale calculations (simplified for demo)
  const scale = height / (high - low) || 1;
  const scaledBodyHeight = bodyHeight * scale;
  const scaledBodyY = y + (high - bodyY) * scale;
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-background border border-border rounded-lg p-3 shadow-lg">
        <p className="text-sm font-medium mb-2">{label ? new Date(label).toLocaleString() : 'N/A'}</p>
        <div className="space-y-1 text-xs">
          ...
          <div className="flex justify-between gap-4">
            ...
            <span className="font-mono text-green-600">${data.high.toFixed(2)}</span>
          </div>
          <div className="flex justify-between gap-4">
            <span className="text-muted-foreground">Low:</span>
            <span className="font-mono text-red-600">${data.low.toFixed(2)}</span>
          </div>
          <div className="flex justify-between gap-4">
...
const CandlestickChart: React.FC<CandlestickChartProps> = ({
  ...
  const generateOHLCData = (symbol: string, range: string): OHLCDataPoint[] => {
    ...
    const pointsMap = {
      '1D': 24, '5D': 120, '1M': 30, '3M': 90, '6M': 180, '1Y': 365
    };
      ...
      case 'candlestick':
        return (
          ...
          >
            ...
            <Bar 
              dataKey={(entry: OHLCDataPoint) => [entry.low, entry.high - entry.low]}
              fill="#8884d8"
              shape={(props: Record<string, unknown>) => <CandlestickBar {...props} />}
            />
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t">
        <div className="flex gap-2">
          {(['line', 'area', 'candlestick', 'volume'] as const).map((type) => (
            ...
            >
              {type === 'candlestick' ? 'OHLC' : type.charAt(0).toUpperCase() + type.slice(1)}
            </button>
          ))}
        </div>
