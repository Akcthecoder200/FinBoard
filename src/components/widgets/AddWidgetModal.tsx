...
export function AddWidgetModal({ isOpen, onClose }: AddWidgetModalProps) {
  ...
  const handleInputChange = (field: keyof WidgetFormData, value: string | number | string[]) => {
  ...
  const isStepValid = () => {
    switch (currentStep) {
      ...
      case 2: return formData.apiUrl.trim().length > 0;
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div>
            <h2 className="text-xl font-semibold text-foreground">Add New Widget</h2>
            <p className="text-sm text-muted-foreground">Configure your dashboard widget</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 bg-secondary/20">
          ...
          <div className="mt-2 text-sm text-muted-foreground">
            Step {currentStep} of {totalSteps}: {
              currentStep === 1 ? 'Widget Details' :
              currentStep === 2 ? 'Data Source' :
              currentStep === 3 ? 'Display Options' : 'Review & Create'
            }
          </div>
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                      Choose a descriptive name that clearly identifies the widget&apos;s purpose on your dashboard.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-lg">
                <div className="flex items-start gap-3">
                  <div className="text-amber-500 mt-0.5">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.896-.833-2.664 0L3.232 16c-.77.833.192 2.5 1.732 2.5z" />
                    </svg>
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                      Make sure your API supports CORS and returns JSON data. Test the URL in your browser first.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { type: 'card', icon: '💰', title: 'Card', desc: 'Summary cards' },
                    { type: 'table', icon: '📊', title: 'Table', desc: 'Data table' },
                    { type: 'chart', icon: '📈', title: 'Chart', desc: 'Visual chart' }
                  ].map(option => (
                    <button
                      key={option.type}
                      onClick={() => handleInputChange('displayType', option.type)}
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <div className="bg-secondary/20 p-4 rounded-lg">
                <h3 className="font-medium text-foreground mb-3">Review Your Widget</h3>
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium capitalize">{formData.displayType}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {REFRESH_INTERVALS.find(i => i.value === formData.refreshInterval)?.label}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Fields:</span>
                    <span className="font-medium">{formData.selectedFields.length} selected</span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-700 dark:text-green-300 mt-1">
                      Your widget configuration looks good! Click &quot;Add Widget&quot; to add it to your dashboard.
                    </p>
                  </div>
                </div>
              </div>
