...
const MarketScanner: React.FC<MarketScannerProps> = ({
  ...
  const [selectedScan, setSelectedScan] = useState<'gainer' | 'loser' | 'volume' | 'breakout' | 'custom'>('gainer');
  const [sortBy, setSortBy] = useState<'change' | 'volume' | 'signal'>('change');
  ...
  const SCAN_UNIVERSE = React.useMemo(() => [
    ...
    'UBER', 'SPOT', 'ZM', 'PYPL', 'ROKU', 'TWLO', 'OKTA', 'SNOW', 'PLTR',
    'COIN', 'HOOD', 'RBLX', 'UPST', 'SOFI', 'LCID', 'RIVN', 'F', 'GM', 'NIO',
    'BABA', 'JD', 'PDD', 'TME', 'BILI', 'IQ', 'BIDU', 'WB', 'TAL', 'INTC',
    'BA', 'CAT', 'DE', 'MMM', 'GE', 'HON', 'LMT', 'RTX', 'UPS', 'FDX'
  ], []);

  // Generate signals based on technical indicators
  const generateSignal = (price: number, change: number, changePercent: number, volume: number, rsi?: number): { signal: ScanResult['signal']; confidence: number } => {
    let score = 0;
    let confidence = 0;
  ...
  const performScan = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < symbolsToScan.length && results.length < maxResults; i += batchSize) {
        ...
        try {
          const batchPromises = batch.map(async (symbol) => {
            try {
              ...
              const rsi = mockRSI();
              const { signal, confidence } = generateSignal(data.price, data.change, data.changePercent, data.volume || 1000000, rsi);
              ...
              return {
                ...
                volume: data.volume || Math.floor(Math.random() * 5000000) + 1000000,
                rsi,
                signal,
                confidence,
      ...
      const filteredResults = results.filter(result => {
        ...
        if (selectedScan === 'volume') return result.volume > scanCriteria.minVolume;
        if (selectedScan === 'breakout') return Math.abs(result.changePercent) > 2;
  ...
  }, [selectedScan, scanCriteria, maxResults, sortBy, sortOrder, SCAN_UNIVERSE]);
  ...
  const getSignalColor = (signal: ScanResult['signal']) => {
    switch (signal) {
      ...
      case 'HOLD': return 'text-gray-500 bg-gray-100';
    ...
    <div className="w-full bg-card rounded-lg border p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <label className="block text-muted-foreground mb-1">Min Volume</label>
                <input
                  type="number"
                  value={scanCriteria.minVolume}
                  onChange={(e) => setScanCriteria(prev => ({ ...prev, minVolume: Number(e.target.value) }))}
                  className="w-full px-2 py-1 border border-border rounded bg-background text-foreground"
                  placeholder="1000000"
                />
              </div>
              <div>
                <label className="block text-muted-foreground mb-1">Change %</label>
                <input
                  type="number"
                  value={scanCriteria.changeThreshold}
                  onChange={(e) => setScanCriteria(prev => ({ ...prev, changeThreshold: Number(e.target.value) }))}
                  className="w-full px-2 py-1 border border-border rounded bg-background text-foreground"
                  placeholder="3"
                />
              </div>
      ...
      <div className="border rounded-lg overflow-hidden">
        <div className="bg-muted/50 px-4 py-3 border-b flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground">
            Last updated: {scanResults[0]?.lastUpdated ? new Date(scanResults[0].lastUpdated).toLocaleTimeString() : 'Never'}
          </span>
        </div>
        ...
        <div style={{ height: height - 200, overflowY: 'auto' }}>
          {loading ? (
            ...
            <div className="flex items-center justify-center h-32">
              <p className="text-muted-foreground">No results found. Try adjusting your scan criteria.</p>
            </div>
          ) : (
              ...
              <tbody>
                {scanResults.map((result, index) => (
                  ...
                  >
                    ...
                    <td className={`px-4 py-3 font-mono ${getChangeColor(result.change)}`}>
                      {result.change >= 0 ? '+' : ''}${result.change.toFixed(2)}
                    </td>
                    <td className={`px-4 py-3 font-mono font-medium ${getChangeColor(result.change)}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`font-mono text-xs ${
                        (result.rsi || 0) > 70 ? 'text-red-500' : 
                        (result.rsi || 0) < 30 ? 'text-green-500' : 'text-muted-foreground'
                      }`}>
                        {result.rsi?.toFixed(0) || '--'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${getSignalColor(result.signal)}`}>
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg">
          <div className="text-green-600 text-sm font-medium">Strong Signals</div>
        ...
        <div className="bg-orange-50 dark:bg-orange-900/20 p-3 rounded-lg">
          <div className="text-orange-600 text-sm font-medium">Avg Confidence</div>
          <div className="text-xl font-bold text-orange-700">
            {scanResults.length > 0 
              ? (scanResults.reduce((sum, r) => sum + r.confidence, 0) / scanResults.length).toFixed(0)
              : 0
            }%
          </div>
        </div>
