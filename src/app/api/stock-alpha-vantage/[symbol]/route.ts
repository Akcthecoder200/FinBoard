  ...
  try {
    ...
    if (requestCount >= RATE_LIMIT) {
      return NextResponse.json(
        {
          success: false,
          error: 'Alpha Vantage rate limit exceeded (5 requests/minute). Try again later.',
    ...
    if (ALPHA_VANTAGE_API_KEY === 'demo') {
      return NextResponse.json(
        {
          success: false,
          error: 'Alpha Vantage demo key has limited functionality. Please use a valid API key.',
          timestamp: new Date().toISOString(),
        },
        { status: 403 }
      );
